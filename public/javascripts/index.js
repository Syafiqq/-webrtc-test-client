(function () {
    ...
    const startChat = async () => {
        try {
            ...
            signaling = new WebSocket(document.getElementById('server-input').value);
    ...
    const addMessageHandler = () => {
        signaling.onmessage = async (message) => {
            ...
            console.log(`Receive message 1: ${JSON.stringify(data)}`)

            if (!data) {
                return;
            }

            let { message_type_, content_ } = data
            if (!message_type_ && !content_) {
                message_type_ = data.text.message_type
                content_ = data.text.content
            }
            const message_type = message_type_
            const content = content_

            console.log(`Receive message 2: ${message_type} ${content}`)
            try {
